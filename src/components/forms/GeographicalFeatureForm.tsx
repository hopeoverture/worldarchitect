...
interface GeographicalFeatureFormProps {
  worldId: string;
  worldContext?: string;
  onClose: () => void;
...
export default function GeographicalFeatureForm({ worldId, worldContext = '', onClose, onSave }: GeographicalFeatureFormProps) {
  ...
  const featureTypes = [
    'forest', 'mountain', 'river', 'lake', 'ocean', 'canyon', 'cave', 'volcano', 'glacier'
  ];
  ...
  const generateWithAI = async () => {
    if (!formData.name || !formData.feature_type) {
      setError('Please enter a name and select feature type first');
      return;
    }
    ...
    setLoading(true);
    try {
      const generatedDescription = await generateGeographicalDescription({
        name: formData.name,
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <h2 className="text-2xl font-bold text-gray-900">Create New Geographical Feature</h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => setFormData(prev => ({ ...prev, feature_type: e.target.value }))}
            >
            ...
            {regions.length === 0 ? (
              <div>
                ...
                <div className="w-full px-4 py-2.5 border border-gray-200 rounded-lg bg-gray-50 text-gray-400">
                  Select region where this feature is located...
                </div>
              </div>
            ) : (
              <select
              ...
              >
                <option value="">Select region where this feature is located...</option>
                {regions.map(region => (
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Describe this geographical feature's appearance, significance, and unique characteristics..."
            />
            {error && (
          ...
          <div className="flex space-x-3 pt-4">
            ...
            >
              ...
              <span>{loading ? 'Saving...' : 'Save Feature'}</span>
            </button>
          </div>
        </form>
      </div>
      ...
      {showLinkModal && (
        <ComponentLinkModal
          ...
          title="Link Components to Geographical Feature"
        />
      )}
    </div>
  );
